...
config = {
    'model_name' : 'vivit-tiny-patch16-224_pv',
    'epochs_total': 1,
    'epochs_froozen': 0,
    'n_inst': 100,
    'train_sz': 0.5,
    ...
    'drop_path_rate' : 0.1,
    'tags' : {
        'Model': 'vivit-tiny-patch16-224_pv',
        'Type': 'Debug',
        'Seeds': [0]
    },
    'eval_dir' : "Evaluation",
...
model = VisionTransformer3D(num_classes=3,img_size=(4,224,224), patch_size=(2,16,16), weight_init=config["pretrained"],
...
train_loader, val_loader, test_loader, inst_dist = build_loader(n_inst=config['n_inst'], seq_len=config["seq_len"], seq=config["seq_len"]>0, 
    fldir="C:/Users/MarkOne/data/regimeclassification", bs=config["batch_size"], device=train_device)
...
learner.pv_learn(config["epochs_total"], params=config["PVs"], n_iter=train_loader.__len__(), loss_we=[0.5, 0.5, 0])
